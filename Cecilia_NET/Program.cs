...
// Speeds up string comparison calls
...
public class Bot
{
    ...
    {
        var logMsg = new LogMessage(severity, source, msg);
        if (writeLine)
        {
            LogAsyncLine(logMsg);
        }
    ...
    public static void Main(string[] args)
    {
        ...
        // Create the configs
        var configPrefix = OsPlatform == OSPlatform.Windows ? @"Config\" : "Config/";
        ...
        {
            // Read in the config
            var rawConfig = File.ReadAllText($@"{configPrefix}bot.json").Replace(Environment.NewLine, "");
            ...
            {
                CreateLogEntry(LogSeverity.Info, "Setup", "Enter a Discord Bot Token (Instructions on Github): ", false)
            ...
            {
                CreateLogEntry(LogSeverity.Info, "Setup", "Enter a Command Prefix (For example -- or !): ", false)
                    .GetAwaiter().GetResult();
        ...
        if (spotifyClientData.ClientId != "" && spotifyClientData.ClientSecret != "")
        {
            if (!(spotifyClientData.ClientId == "-1" || spotifyClientData.ClientId == "-1"))
            {
                ...
                SpotifyConfig = SpotifyClientConfig
                    .CreateDefault()
                    .WithAuthenticator(
                        new ClientCredentialsAuthenticator(spotifyClientData.ClientId, spotifyClientData.ClientSecret));
                // Notify success
                CreateLogEntry(
                    LogSeverity.Info,
                    "Spotify",
        ...
        else
        {
            ...
            while (input == "")
            {
                ...
                CreateLogEntry(LogSeverity.Info, "Spotify", "Would you like to setup spotify integration? Y/N: ", false)
                    .GetAwaiter().GetResult();
                ...
                if (input == "y" || input == "yes")
                {
                    ...
                    {
                        CreateLogEntry(LogSeverity.Info, "Spotify", "Enter a client ID: ", false).GetAwaiter()
                            .GetResult();
                    ...
                    {
                        CreateLogEntry(LogSeverity.Info, "Spotify", "Enter the client secret: ", false).GetAwaiter()
                            .GetResult();
                    ...
                    SpotifyConfig = SpotifyClientConfig
                        .CreateDefault()
                        .WithAuthenticator(
                            new ClientCredentialsAuthenticator(spotifyData.ClientId, spotifyData.ClientSecret));
                    // Notify success
                    CreateLogEntry(
                        LogSeverity.Info,
                        "Spotify",
        ...
        {
            var di = new DirectoryInfo("AudioCache");
